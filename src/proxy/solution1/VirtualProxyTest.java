...
public class VirtualProxyTest {
    ...
    @Test
    public void testForObjectMethods() {
        Collection<String> missingMethods = new ArrayList<>();

        Class<?> virtualLutefiskClass = findVirtualLutefiskClass();
        try {
            virtualLutefiskClass.getDeclaredMethod("hashCode");
        } catch (NoSuchMethodException e) {
            missingMethods.add("hashCode");
        }
        try {
            virtualLutefiskClass.getDeclaredMethod("toString");
        } catch (NoSuchMethodException e) {
            missingMethods.add("toString");
        }
        try {
            virtualLutefiskClass.getDeclaredMethod("equals", Object.class);
        } catch (NoSuchMethodException e) {
